...
namespace FAS.FirehousePro.Tests
{
    public abstract class FirehouseProTestBase : AbpIntegratedTestBase<FirehouseProTestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected FirehouseProTestBase()
        {
        ...
        protected void UsingDbContext(Action<FirehouseProDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<FirehouseProDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<FirehouseProDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<FirehouseProDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<FirehouseProDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<FirehouseProDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<FirehouseProDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<FirehouseProDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<FirehouseProDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<FirehouseProDbContext, Task<T>> func)
        {
            T result;
